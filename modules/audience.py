from flask import Blueprint, redirect, request, render_template_string, url_for, session
import requests
import hashlib
import hmac
import pandas as pd
...
def home():
    ...
    <div class='container'>
        <div class='card p-4 text-center'>
            ...
            <a href='/fb/login/facebook' class='btn btn-primary'>Login with Facebook</a>
        </div>
    </div>
    </body></html>
    """

@fb_blueprint.route("/login/facebook")
def facebook_login():
    fb_oauth_url = (
...
@fb_blueprint.route("/auth/callback")
...
@fb_blueprint.route("/create_audience", methods=["POST"])
def create_audience():
    ...
    upload_link = url_for("fb.upload_data")
...
@fb_blueprint.route("/upload_data", methods=["GET", "POST"])
def upload_data():
    access_token = session.get("access_token")
    if not access_token:
        return redirect(url_for("fb.home"))
    ...
    if request.method == "POST":
        ...
        try:
            df = pd.read_csv(file)
            expected_columns = ['FN', 'LN', 'PHONE']  # Facebook expected columns
            ...
            <div class='container'>
                <div class='card p-4'>
                    ...
                    <pre>{response}</pre>
                    <a href='/fb/upload_data' class='btn btn-primary mt-3'>Upload More Data</a>
            ...
            <div class='container'>
                <div class='card p-4'>
                    ...
                    <p>{str(e)}</p>
                    <a href='/fb/upload_data' class='btn btn-primary mt-3'>Try Again</a>
                    <a href='/' class='btn btn-secondary mt-3'>Home</a>
                </div>
            </div>
            </body></html>
